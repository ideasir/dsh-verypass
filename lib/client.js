      ...
      <div class="dsh-verypass-edit-footer" style="border:none;padding-top:4px">
        ...
        <button class="dsh-verypass-btn-pri" style="background:var(--dsw-alias-state-error-primary,#ef4444)" data-confirm-action="ok">确定</button>
				...
				el.addEventListener("click", (e) => {
					...
					const b = e.target.closest("[data-confirm-action]");
					if (!b) return;
					el.remove();
		...
		function delProject(project, overlay) {
			const entries = secretsData.secrets.filter((s) => (s.project || "默认") === project);
			if (entries.length === 0) return;
			confirmModal(`确定删除整个项目「${escapeHtml(project)}」（${entries.length} 个 Key）？`).then((ok) => {
				if (!ok) return;
				entries.forEach((e) => {
		...
		function delSecret(idx, overlay) {
			if (idx < 0 || idx >= secretsData.secrets.length) return;
			confirmModal(`确定删除「${escapeHtml(secretsData.secrets[idx].name)}」？`).then((ok) => {
				if (!ok) return;
			...
			const submit = () => {
				...
				for (const val of rawValues) {
					const formForGen = {
						...form,
						value: val,
						name: rawValues.length > 1 ? `${name} ${created.length + 1}` : name,
						variable: ""
					};
				...
				if (isEdit) {
					...
					const nowStr = now();
					created.forEach((c) => secretsData.secrets.push({
						...c,
						createdAt: nowStr
					}));
				}

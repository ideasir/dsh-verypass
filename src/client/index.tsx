...
interface SecretEntry {
  ...
  project: string     // 所属项目（如"Agnes API"）
  variable: string    // 变量名（如"ARM_SSH_KEY"）
  prefix: string      // 变量前缀（用户填写；空则系统自动生成）
...
function saveData() {
  ...
  try {
    void fetch('/plugins/dsh-verypass/save', {
...
const ChevronSvg = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="12" height="12"><polyline points="6 9 12 15 18 9"/></svg>'
...
function groupByProject(): Array<{ project: string; entries: SecretEntry[] }> {
...
function renderVaultListHtml(): string {
...
function openVarlist(project: string) {
  ...
  el.addEventListener('click', (e) => {
    ...
    const b = (e.target as HTMLElement).closest('[data-varlist-action]')
    if (b) el.remove()
  })
}
...
function openStats() {
  ...
  const rows = [...byProject.entries()]
    .sort((a, b) => (a[0] === '默认' ? 1 : b[0] === '默认' ? -1 : a[0].localeCompare(b[0])))
    .map(([project, count]) => `<tr>
  ...
  el.addEventListener('click', (e) => {
    if (e.target === el) { el.remove(); return }
    const b = (e.target as HTMLElement).closest('[data-stats-action]')
    if (b) el.remove()
  })
}
...
/** 自定义确认弹窗（替代原生 confirm，DSH webview 中原生 confirm 被拦截） */
function confirmModal(msg: string): Promise<boolean> {
  return new Promise(resolve => {
    const el = document.createElement('div')
    el.style.cssText = 'position:fixed;inset:0;z-index:2147483003;background:rgba(0,0,0,0.55);display:flex;align-items:center;justify-content:center;animation:dsh-verypass-fadein .18s ease-out'
    el.innerHTML = `<div class="dsh-verypass-edit-box" style="width:400px">
      <div class="dsh-verypass-edit-header" style="border:none;padding-bottom:0">
        <span style="font-size:14px;font-weight:500;color:var(--dsw-alias-label-primary,#fff)">${escapeHtml(msg)}</span>
      </div>
      <div class="dsh-verypass-edit-footer" style="border:none;padding-top:4px">
        <button class="dsh-verypass-btn-sec" data-confirm-action="cancel">取消</button>
        <button class="dsh-verypass-btn-pri" style="background:var(--dsw-alias-state-error-primary,#ef4444)" data-confirm-action="ok">确定</button>
      </div>
    </div>`
    document.body.appendChild(el)
    el.addEventListener('click', (e) => {
      if (e.target === el) { el.remove(); resolve(false); return }
      const b = (e.target as HTMLElement).closest('[data-confirm-action]')
      if (!b) return
      el.remove()
...
function delSecret(idx: number, overlay: HTMLElement) {
  if (idx < 0 || idx >= secretsData.secrets.length) return
  void confirmModal(`确定删除「${escapeHtml(secretsData.secrets[idx].name)}」？`).then(ok => {
    if (!ok) return
...
function openEditModal(editIdx: number | null, overlay: HTMLElement) {
  ...
  const entry = isEdit ? secretsData.secrets[editIdx] : { name: '', project: '', variable: '', prefix: '', value: '', note: '', createdAt: '' }
  ...
  const readForm = () => {
    ...
    return { name: g('name').trim(), project: g('project').trim(), variable: g('variable').trim(), value: g('value'), note: g('note').trim() }
  }

  // 自动生成变量名：变量名为空时，从名字生成（大写+下划线），同名前缀追加序号
  const genVariable = (form: ReturnType<typeof readForm>): string => {
    if (form.variable) return form.variable
    let p = form.name.toUpperCase().replace(/[^A-Z0-9]+/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '')
    if (!p) p = 'KEY'
    const same = secretsData.secrets.filter(s => s.variable.startsWith(p + '_') || s.variable === p)
    let n = same.length + 1
    let cand = `${p}_${String(n).padStart(2, '0')}`
    while (secretsData.secrets.some((s, i) => s.variable === cand && i !== editIdx)) { n++; cand = `${p}_${String(n).padStart(2, '0')}` }
    return cand
  }

  const varInput = editDiv.querySelector('[data-field="variable"]') as HTMLInputElement
  const hintEl = editDiv.querySelector('[data-hint]') as HTMLElement
  const updateHint = () => {
    const f = readForm()
    if (!f.variable && f.name) {
      hintEl.textContent = `将自动生成变量名：${genVariable(f)}`
    } else {
      hintEl.textContent = ''
    }
  }
  varInput.addEventListener('input', () => { if (!varInput.value) updateHint() })
...
export function apply(ctx: any) {
  ...
  void fetch('/plugins/dsh-verypass/list', { cache: 'no-store' }).then(r => r.json()).then((data: any) => {
...
`
  ...
  // key = verypass，匹配服务端 settingsNamespace('verypass')
...
function LockButton() {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('dsh-verypass-toggle', handler)
    return () => window.removeEventListener('dsh-verypass-toggle', handler)
  }, [])
  ...
  return React.createElement('button', {
    ...
    className: 'dsh-verypass-lock-btn' + (enabled ? ' dsh-verypass-lock-on' : ''),
...
function VeryPassPluginCard() {
  ...
  return React.createElement('li', { className: 'dsh-mm-card' },
    ...
    React.createElement('button', { className: 'dsh-mm-head', onClick: () => setOpen(v => !v) },
      React.createElement('span', { className: 'dsh-mm-head-text' },
        React.createElement('div', { className: 'dsh-mm-name-row' },
          React.createElement('span', { className: 'dsh-mm-title', style: { display: 'inline-flex', alignItems: 'center', gap: 6 } }, React.createElement('span', { dangerouslySetInnerHTML: { __html: LockSvg } }), 'VeryPass'),
          React.createElement('span', { className: 'dsh-mm-version-badge' }, VERSION),
        ),
        React.createElement('span', { className: 'dsh-mm-desc' }, '管理敏感凭证的密码本。'),
      ),
      React.createElement('span', { className: 'dsh-mm-btns' },
        React.createElement('a', { className: 'dsh-mm-btn-link', href: REPO_URL, target: '_blank', rel: 'noreferrer', onClick: (e: any) => e.stopPropagation(), title: '打开 GitHub 仓库',
        ...
        React.createElement('button', { type: 'button', className: 'dsh-mm-btn-uninstall', onClick: (e: any) => { e.stopPropagation(); if (confirm('确定卸载 VeryPass 插件？')) { setFeedback('已卸载（重启后生效）'); setTimeout(() => setFeedback(null), 3000) } }, title: '卸载插件',
    ...
    open && React.createElement('div', { className: 'dsh-mm-body' },
      ...
      // 开关 + 状态（放在一个带边框的容器卡片里，开关在左，说明在右）
      React.createElement('div', { className: 'dsh-verypass-status-card' },
        React.createElement('button', {
          ...
          React.createElement('span', {
            style: {
              ...
              width: 18, height: 18, borderRadius: 999, background: 'var(--dsw-alias-label-primary-inverted)',
              boxShadow: 'var(--dsw-alias-shadow, 0 1px 3px rgba(0,0,0,0.3))',
              transition: 'left .2s cubic-bezier(0.34, 1.56, 0.64, 1)',
            } as any,
          }),
        ),
        React.createElement('div', { className: 'dsh-verypass-status-info' },
          React.createElement('div', { className: 'dsh-verypass-status-title' }, enabled ? 'VeryPass 已开启' : 'VeryPass 已关闭'),
          React.createElement('div', { className: 'dsh-verypass-status-note' }, enabled ? 'AI 可以调用 resolve_secret 工具获取脱敏值' : '关闭后 AI 无法调用密码本工具'),
        ),
      ),
      // 打开密码本按钮（跟随主题色）
      React.createElement('button', {

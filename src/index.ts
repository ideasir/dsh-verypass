...
interface SecretEntry {
  ...
  project: string     // 所属项目（如 "Agnes API"；空则归入"默认"）
  variable: string    // 变量名（如"ARM_SSH_KEY"），唯一
  prefix: string      // 变量前缀（用户填写；空则系统按项目自动生成变量名）
...
export function apply(ctx: any, config: any = {}) {
    ...
    ctx.webServer.register({
      ...
      handler: async (req: IncomingMessage, res: ServerResponse) => {
        try {
          ...
          const submitted = Array.isArray(data.secrets) ? data.secrets.filter((r: any) => r && typeof r === 'object') : []
  ...
  const registerTools = () => {
    ...
    toolDisposers.push(ctx.tools.register(defineTool({
      ...
      async execute(): Promise<{ entries: Array<{ project?: string; name: string; variable: string; prefix?: string; masked: string; note?: string }> }> {
        assertEnabled()
        const data = await loadData()
        return {
    ...
    toolDisposers.push(ctx.tools.register(defineTool({
      ...
      async execute(args: { variable: string }): Promise<{ found: boolean; name?: string; project?: string; variable?: string; prefix?: string; masked?: string; note?: string }> {
        assertEnabled()
        const data = await loadData()
